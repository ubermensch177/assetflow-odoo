...
export function AllocationModal({ isOpen, onClose, onAllocate }: AllocationModalProps) {
  const [assignedTo, setAssignedTo] = useState('');
  const [department, setDepartment] = useState('');
  const [expectedReturn, setExpectedReturn] = useState('');
  const [conditionOnOut, setConditionOnOut] = useState('Good');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          <h3 className="font-semibold text-slate-800">Allocate Asset</h3>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">Assign To (Employee) *</label>
            <input 
          ...
          <div>
            ...
            <input 
              ...
              type="text" className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Remote Work" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">Notes</label>
            <textarea 
              value={notes} onChange={e => setNotes(e.target.value)}
              className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" placeholder="Additional details..." rows={2} />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">Condition on Out</label>
            <select 
              value={conditionOnOut} onChange={e => setConditionOnOut(e.target.value)}
          ...
          <button 
            onClick={() => onAllocate({ assignedTo, department, expectedReturn, purpose, notes, conditionOnOut })}
